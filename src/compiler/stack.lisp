...
;;; determine which unknown-values continuations are on the stack at each
;;; point in the program, and then we insert cleanup code to pop off unused
;;; values.
...
#[ Stack Analysis
    ...
    presence of local exits.

Phase position: 13/23 (middle)

Presence: required

Files: stack

Entry functions: `stack-analyze'

Call sequences:

    native-compile-component
      stack-analyze
        find-values-generators
        find-pushed-continuations
          stack-simulation-walk
          annotate-dead-values
          discard-unused-values


Stack analysis does a graph walk to determine which unknown-values
continuations are on the stack at each point in the program, and inserts
...
;;;
;;; If we run into a values generator whose continuation isn't on stack
;;; top, then the receiver hasn't yet been reached on any walk to this use.
;;; In this case, we ignore the push for now, counting on
...
;;; that don't have all the continuations that we have on our Start-Stack
;;; on their End-Stack.  This is our termination condition for the graph
;;; walk.  We put the test around the recursive call so that the initial
;;; call to this function will do something even though there isn't
;;; initially anything on the stack.
;;;
;;; We can use the tailp test, since the only time we want to bottom out
;;; with a non-empty stack is when we intersect with another path from the
;;; same top-level call to this function that has more values receivers on
;;; that path.  When we bottom out in this way, we are counting on
;;; `discard-unused-values' doing its thing.
;;;
;;; When we do recurse, we check that predecessor's END-STACK is a
;;; subsequence of our START-STACK.  There may be extra stuff on the top of
;;; our stack because the last path to the predecessor may have discarded
...
;;; unreached by all walks (i.e. the continuation isn't live at the point
;;; that it is generated.)  This will only happen when the values receiver
;;; cannot be reached from this particular generator (due to an
;;; unconditional control transfer.)
;;;
;;; What we do is push on the End-Stack all continuations in Pushed that
;;; aren't already present in the End-Stack.  When we find (FIX last loop in `stack-analyse'?) any pushed
;;; continuation that isn't live, it must be the case that all
;;; continuations pushed after (on top of) it aren't live.
;;;
;;; If we see a pushed continuation that is the CONT of a tail call, then
;;; we ignore it, since the tail call didn't actually push anything.  The
;;; tail call must always be the last in the block.
;;;
...
;;; LTN-ANALYZE-BLOCK on the new block.  The new block is inserted after
;;; Block1 in the emit order.
;;;
;;; If the control transfer between Block1 and Block2 represents a
;;; tail-recursive return (:Deleted IR2-continuation) or a non-local exit,
;;; then the cleanup code will never actually be executed.  It doesn't seem
;;; to be worth the risk of trying to optimize this, since this rarely
;;; happens and wastes only space.
;;;
...
;;; cleanup code to discard values that are generated but never received.
;;; This phase doesn't need to be run when Values-Receivers is null, i.e.
;;; there are no unknown-values continuations used across block boundaries.
;;;
;;; Do the backward graph walk, starting at each values receiver.  We
;;; ignore receivers that already have a non-null Start-Stack.  These are
;;; nested values receivers that have already been reached on another walk.
;;; We don't want to clobber that result with our null initial stack.
;;;
