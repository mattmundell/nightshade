;;; A functional interface to global information about named things in the
;;; system.  Information is considered to be global if it must persist
;;; between invocations of the compiler.  The use of a functional interface
;;; eliminates the need for the compiler to worry about the actual
;;; representation.  This is important, since the information may well have
;;; several representations.  This code also deals with the need for
;;; multiple "global" environments, so that changing something in the
;;; compiler doesn't trash the running Lisp environment.
;;;
;;; The database contains arbitrary Lisp values, addressed by a combination
;;; of Name, Class and Type.  The Name is a `equal'-thing which is the name
;;; of the thing we are recording information about.  Class is the kind of
;;; object involved.  Typical classes are Function, Variable, Type, ....  A
;;; Type names a particular piece of information within a given class.
;;; Class and Type are symbols, and are compared with `string='.

;; FIX add above to doc
...
(defstruct (type-info
  ...
  (default #'(lambda () (error "Type yet to be defined.")) :type function))
...
;;; actually store the init function until load time so that we don't break
...
;;;
;;; If there is no info for the class, then create it, otherwise do
;;; nothing.
;;;
...
(defun find-unused-type-number ()
  (or (position () *type-numbers*)
      (error "Out of INFO type numbers.")))

;;; Define-Info-Type  --  Public
;;;
;;; The main thing we do is determine the type's number.  We need to do
;;; this at macroexpansion time, since both the COMPILE and LOAD time calls
;;; to %DEFINE-INFO-TYPE must use the same type number.
;;;
(defmacro define-info-type (class type type-spec &optional default)
  "define-info-type class type default type-spec
   ...
   name of the type, Default is the value for that type when it hasn't been
   set, and Type-Spec is a type-specifier which values of the type must
   satisfy.  The default expression is evaluated each time the information
   is needed, with Name bound to the name for which the information is
   being looked up.  If the default evaluates to something with the second
   value true, then the second value of Info will also be true."
...
;;;; Info environments.
;;;
;;; We do info access relative to the current *info-environment*.  This is
;;; a list of INFO-ENVIRONMENT structures we search.  The variable is
;;; actually initialized in GLOBALDB-INIT.
...
;;; always returns the same value as SXHASH, just that it satisfies the
;;; formal definition of SXHASH.  The ``sorta'' is because symbol-hash will
;;; not necessarily return the same value in different lisps.
;;;
...
;;; does, but we get there more expeditiously.  With the two-list, we
;;; LOGXOR a random constant with the hash of the second symbol.
;;;
...
;;; number at compile time.  When we check the new-value's type directly in
;;; the SETF expansion, FIX? since the check can be done much more
;;; efficiently when the type is constant.
;;;
(defmacro info (class type name &optional env-list)
  "Return the information of the $type and $class for $name.  Return the
   second value true if there is any such information recorded, otherwise
   return the fallback as the first value and the second value ()."
  ;;
...
(defmacro do-info ((env &key (name (gensym)) (class (gensym)) (type (gensym))
  ...
  "do-info (env &key name class type value) form*
   ...
   Iterate over all the values stored in the Info-Env Env.  Name is bound
   to the entry's name, Class and Type are bound to the class and type
...
;;; of *INFO-ENVIRONMENT*.  This is in addition to the per-environment
;;; caching of name => types.
...
;;; allowed, we don't have to worry about deleted entries.  We indirect
;;; through a parallel vector to find the index in the ENTRIES at which the
;;; entries for a given name starts.
;;;
...
;;;
;;; Iterate over the environment once to find out how many names and
;;; entries it has, then build the result.  This code assumes that all the
;;; entries for a name well be iterated over contiguously, which holds true
;;; for the implementation of iteration over both kinds of environments.
;;;
;;; When building the table, we sort the entries by POINTER< in an attempt
;;; to preserve any VM locality present in the original load order, rather
;;; than randomizing with the original hash function.
;;;
...
;;;
;;; Given a volatile environment Env, bind Table-Var the environment's
;;; table and Index-Var to the index of Name's bucket in the table.  We
;;; also flush the cache so that things will be consistent if body modifies
;;; something.
;;;
...
;;; environment's table.  We clear the old table to prevent it from holding
;;; onto garbage if it is statically allocated.
;;;
...
;;;
;;; Check if the name and type is in our cache, if so return it.
;;; Otherwise, search for the value and encache it.
;;;
;;; Return the value from the first environment which has it defined, or
;;; return the default if none does.  We have a cache for the last name
;;; looked up in each environment.  We don't compute the hash until the
;;; first time the cache misses.  When the cache does miss, we invalidate
;;; it before calling the lookup routine to eliminate the possiblity of the
;;; cache being partially updated if the lookup is interrupted.
;;;
...
;;;
;;; Since the global enviornment database is used by top-level forms in
;;; this file, we must initialize the database before processing any
;;; top-level forms.  This requires a special initialization function that
;;; is called from %INITIAL-FUNCTION.  We replicate the init forms of the
;;; variables that maintain the class/type namespace.  We also initialize
;;; the info cache.
;;;
(defun globaldb-init ()
  (or (boundp '*info-environment*)
