...
There is an archive available at

    http://www.mundell.ukfsn.org/nightshade/nightshade-1f.tar.bz2

for installation on GNU/Linux 32bit x86 systems.

This is a bzip2 tar file that includes binary and source and is intended
for installation to /usr/local/.  The following commands expand the archive

    > cd /usr/local/
    > tar xvf /path/to/dist/nightshade-1f.tar.bz2

The site initialization file is then available for modification at
lib/nightshade/site-init.lisp relative to the installation directory
(/usr/local/lib/nightshade/site-init.lisp in the example above).
...
Building the system is currently quite a rough process.  Prebuilt binary
packages with source are available as an alternative, as described in
[Installation].

Two things are needed to build the system: the source and an already
working system (the builder).  The builder probably needs to be the same
release as the source, too.

The Nightshade executable is a a small C program which loads a large
prepared system image into memory and sets that image running.  This
kind of image is a "core".
...
Throughout this documentation the following terms are used:
  ...
  % Core file
    ...
    file.  The C code includes the X window system version 10 release 4
    which may be called from Lisp.

  % Cold core file

    A cold core file contains enough of the Lisp system to make it
...
The build process involves building the small C program, compiling the
sources with the builder, building a minimal "kernel" core, and then
loading the compiled sources into the kernel core to create the final core.
...
The editor requires a termcap file, which can be generated with a Unix
shell command like

    for f in `find /lib/terminfo/`; do echo; infocmp -Cr `basename $f` 2> /dev/null; done

or downloaded from

    http://www.mundell.ukfsn.org/termcap.bz2.
...
FIX these expect build:*xxx-dir*
...
#[ Compiling the C
    ...
    FIX
    detail somewhere how to setup builder
    in build/lisp link Config to ../../src/lisp/Config.xxx
    in build/lisp link Makefile to ../../src/lisp/GNUMakefile
    check config

Evaluating

    (build:build "n:src/" "c-core")

compiles the C core.  This simply runs

    $ make all

in n:build/lisp.

There is a circular dependancy between lisp/internals.h and lisp/lisp.map
that causes bootstrapping problems.  One way to get around this problem is
to make a fake lisp.nm file that has nothing in it by a version number:

    $ echo "Map file for lisp version 0" > lisp.nm

FIX just run nm instd of next step?

and then run genesis with () for the list of files:

    * (load ".../compiler/generic/new-genesis") ; compile before loading
    * (lisp::genesis () ".../lisp/lisp.nm" "/dev/null"
                     ".../lisp/lisp.map" ".../lisp/lisp.h")

It will generate a whole bunch of warnings about things being undefined, as
well as a correct lisp.h.  Then compile lisp to produce a correct lisp.map,
as above
...
This invokes the build-world.lisp tool, using the following Lisp fragment

    (setf user::src "FIX")
    (setf user::target "FIX")
