...
(defun fixup-code-object (code offset fixup kind)
  ...
  (flet ((add-fixup (code offset)
	   ...
	   ;; objects in the read-only and static spaces, this should only
	   ;; be the case when *enable-dynamic-space-code* is True.
...
;;;
;;; Add a code fixup to a code object generated by new-genesis. The fixup
;;; has already been applied, it's just a matter of placing the fixup in
;;; the code's fixup vector if necessary.
;;;
#+gencgc
...
;;;
;;; An escape register saves the value of a register for a frame that
;;; someone interrupts.
...
;;; Not supported on FreeBSD because the floating point state is not saved.
#+FreeBSD
(defun sigcontext-float-register (scp index format)
...
;;;
;;;    Given a sigcontext pointer, return the floating point modes word in
...
;;; FLOAT-WAIT
;;;
;;; This is used in error.lisp to insure floating-point exceptions are
;;; properly trapped. The compiler translates this to a VOP.
;;;
(defun float-wait()
  (float-wait))
...
;;; i387 load constant instructions to avoid consing in some cases. Note
;;; these are initialise by genesis as they are needed early.
;;;
...
;;; Support for the MT19937 random number generator. The update function is
;;; implemented as an assembly routine. This definition is transformed to a
;;; call to this routine allowing its use in byte compiled code.
;;;
