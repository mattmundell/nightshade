...
(defun complete-string (string tables)
  ...
  (let ((separator (string-table-separator (car tables))))
	...
	(multiple-value-bind
	    (str ambig-pos unique-p last-field-p)
	  ...
	  (multiple-value-bind (value found-p) (find-values str tables)
	    (let ((field-pos (compute-field-pos string str separator)))
	      (cond ((not found-p)
		     (values str :ambiguous nil field-pos ambig-pos last-field-p))
		    (unique-p
		     (values str :unique value field-pos nil t))
		    (t
		     (values str :complete value field-pos ambig-pos last-field-p))))))))))
...
(defun find-longest-completion (strings separator)
  ...
  (let ((first (car strings))
    ...
    (let ((unique-p (and (null ambig-pos)
			 maybe-unique-p
			 (every #'(lambda (x) (null (cdr x))) rest-strings))))
      (values
       (subseq *complete-string-buffer* 0 buf-pos)
       ;; If every corresponding field in each possible completion was
       ;; equal, our result string is an initial substring of some other
       ;; completion, so we're ambiguous at the end.
       (or ambig-pos buf-pos)
       unique-p
       (or unique-p
	   ;; FIX How should this be done?
	   ;; Either the completion includes some of the last possible
	   ;; field or there is a seperator after buf-pos in at least one
	   ;; of the strings.
	   (dolist (s strings t)
	     (when (position separator (if (consp s) (car s) s)
			     :start buf-pos :test #'char=)
	       (return nil))))))))

(defun find-longest-completion2 (strings separator)
  (declare (base-char separator))
  (let ((first (car strings))
	(rest-strings (cdr strings))
	(punt-p nil)
