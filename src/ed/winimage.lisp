...
;; FIX This is an attempt to solve duplication of the screen line following
;;     transposed lines on a tty.  It may need more work and it is likely
;;     that it could be done in a cleaner way.
; orig
	   (setq delta (- pos (1+ (dis-line-position (car prev)))))
; 	   (incf net-delta delta)
; 	   (setq delta (- pos (+ (1+ (dis-line-position (car prev)))
; ;				 net-delta)))
; 				 delta)))
; 	   (incf net-delta delta)
...
;; FIX may need more for net-delta? delta set already to incl net-delta
...
;; For tracing into redisplay.
(defvar *trace-stream* nil)
(defun trc (&rest args)
  (when *trace-stream*
    (apply 'format *trace-stream* args)))

#|
(defun move-lines2 (window changed string underhang line offset trail current
			  width)
  (trc "m-l2~%")
  (do* ((delta 0)
;	(net-delta 0)
    ...
    (trc "pos ~A delta ~A~%" pos delta)
    (trc "cc: ~A~%" cc)
    (trc "old-line: ~A~%" old-line)
    (trc "current: ~A~%" current)
;     (when *trace-stream*
;       (dolist (dl (window-first-line (current-window)))
; 	(format *trace-stream* "dl: .~S.~%"
; 		(if (zerop (length (dis-line-chars dl)))
; 		    ""
; 		    (subseq (dis-line-chars dl) 0 30)))))
;     (when *trace-stream*
;       (loop for l = (mark-line (buffer-start-mark (current-buffer)))
; 	          then (line-next l)
; 	    while l
; 	    do
; 	(format *trace-stream* "bl: .~S.~%"
; 		(line-string l))))
    ...
    (cond
     ((neq (line-%buffer old-line) buffer)
      ...
      ;; FIX it must increment pos?  maybe sort of


      (do ((ptr (cdr current) (cdr ptr))
	   (prev current ptr))
	  ((eq (line-%buffer (dis-line-line (car ptr))) buffer)
	   (trc "set delta:~%pos: ~A~%" pos)
	   (trc "(car prev): ~A~%" (car prev))
	   (trc "(car ptr): ~A~%" (car prev))
;	   (trc "net-delta: ~A~%" net-delta)
	   (trc "pre delta: ~A~%" delta)
	   ;; FIX so if one line is deleted delta stays 0? depends on pos,dis-line-pos
; FIX orig
	   (setq delta (- pos (1+ (dis-line-position (car prev)))))
; 	   (incf net-delta delta)
; 	   (setq delta (- pos (+ (1+ (dis-line-position (car prev)))
...
;	(decf pos)
      ...
      (trc "delta after: ~A~%" delta)
      (setq cc (car current)  old-line (dis-line-line cc)))
     ;; If the line-number of the old line is less than the line-number
     ;; of the line we want to display then the old line must be off the top
     ;; of the screen (FIX or above the line?) - delete it.  The-Sentinel fails this test because
     ;; it's line-number is most-positive-fixnum.
     ((< (line-number old-line) new-num)
      ...
      (do ((ptr (cdr current) (cdr ptr))
	   (prev current ptr))
	  ((>= (line-number (dis-line-line (car ptr))) new-num)
	   ;; FIX if this directly follows the previous case, then the delta will be overwritten
	   ;;         which is ok because pos is the same
;; FIX orig
	   (setq delta (- pos (1+ (dis-line-position (car prev)))))
; 	   (trc "net-delta: ~A~%" net-delta)
; 	   (trc "pre delta: ~A~%" delta)
...
;	(decf pos)
	...
	(setf (dis-line-flags cc) (logior (dis-line-flags cc) changed-bit)
	      ...
	      (dis-line-position cc) pos)
	(unless (zerop delta)
	  ;; FIX the delta is 1 because the line has moved (it used to hold g).
	  ;;     the chars (h) are in the same posn on the window (in this moved line).
	  ;;     does setting the delta 1 on the dis-line cause the line to
	  ;;     be moved on the screen? (and hence cause the display
	  ;;     duplication?)
	  ;; FIX why set the delta when the line has changed? prhps because dis-line has moved?
	  ;;         psbly because some of the line may be the same
	  (incf (dis-line-delta cc) delta)
	  (setf (dis-line-flags cc) (logior (dis-line-flags cc) moved-bit)))
	(shiftf trail current (cdr current))
	(setq cc (car current)  old-line (dis-line-line cc)  pos (1+ pos))
...
(defmacro maybe-change-window2 (window changed line offset trail current width)
  `(let* ((cc (car ,current))
     ...
     (cond
      ...
      ((and (null (dis-line-old-chars cc))
	    (eq old-line (dis-line-line (car ,trail))))
       ;; FIX maybe for wrapped lines dis-line-old-chars is nil
       (trc "cont~%")
       (do ((ptr (cdr ,current) (cdr ptr))
	    (prev ,current ptr))
	   ((not (eq (dis-line-line (car ptr)) old-line))
      ...
      ;; A line is changed.
      ((eq old-line ,line)
       ;; FIX same line (here ~ because chars changed from dis-line chars)
       (trc "line change~%")
       (unless ,changed
	 (when (< (dis-line-position cc)
		  (dis-line-position (car (window-first-changed ,window))))
	 ...
	 (cond
	  ((eq (dis-line-line cc) ,line)
	   ;; FIX next line (line of next dis-line) same as line
	   (unless underhang
	     ;; FIX think this happens in transpose case (was that the intention here?)
	     ;; FIX mv lines following next line to be at dis-line current?
	     (move-lines2 ,window t nil 0 (line-next ,line) 0 ,trail ,current
			 ,width)
	     (return t)))
	  (underhang
	   (move-lines2 ,window t string underhang ,line start ,trail
		       ,current ,width)
	   (return t))
	  (t
	   ;; FIX changed a dis-line holding an entire single line which
	   ;; still holds the entire single line, hence nil return
	   ;; (upd-win-img can,must check rest)
	   (setq ,line (line-next ,line))
	   (when (> (dis-line-position (car ,trail))
		    (dis-line-position (car (window-last-changed ,window))))
	     (setf (window-last-changed ,window) ,trail))
      ...
      (t
       (trc "mv lines~%")
       (move-lines2 ,window ,changed nil 0 ,line ,offset ,trail ,current
		   ,width)
       t))))

(defun update-window-image2 (window)
  (trc "u-w-i~%")
  (let* ((trail (window-first-line window))
	 (current (cdr trail))
	 (display-start (window-display-start window))
	 (line (mark-line display-start))
	 (width (window-width window)) changed)
    (trc "lines before update:~%")
    (loop for l = line then (line-next l)
          for dl = trail then (cdr dl)
          while (and l (car dl)) do
      (let ((dll (dis-line-line (car dl))))
	(trc "dis-line (~A): ~A~%" (if dll (line-number dll)) (car dl)))
      (trc "l ~A: ~A~%" (line-number l) l))
    (cond
     ;; If the first line or its charpos has changed then bug out.
     ((cond ((and (eq (dis-line-old-chars (car current)) (line-%chars line))
		  (mark= display-start (window-old-start window)))
	    ...
	    (t
	     ...
	     (let ((start-charpos (mark-charpos display-start)))
	       (move-mark (window-old-start window) display-start)
	       (maybe-change-window2 window changed line start-charpos
				    trail current width)))))
     (t
      (prog ()
	(go TOP)
	...
	(cond ((null line)
	       (go DONE))
	      ;; FIX key here is that if line changes line-chars object changes
	      ;;     otherwise stays same (dis-line chars stays the same, original
	      ;;     chars, either way)
	      ((eq (line-%chars line) (dis-line-old-chars (car current)))
	       (go STEP)))
	;;
	...
	(when (and (eq current the-sentinel)
	  ...
	  (return nil))
	(trc "t2~%")
	(when (maybe-change-window2 window changed line 0 trail current width)
	  (return nil))
	(go TOP)
	...
	;; We hit the end of the buffer. If lines need to be deleted bug out.
	(unless (eq current the-sentinel)
	  (maybe-change-window2 window changed line 0 trail current width))
	(return nil))))
    ;;
    ;; Update the display-end mark.
    (let ((dl (car (window-last-line window))))
