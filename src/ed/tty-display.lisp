...
;;; between the last block of lines that moved with the same delta (FIX ie the delta that was used for that block of lines?) and the
...
(defun compute-tty-changes (first-changed last-changed modeline-pos)
  ...
  (let* ((dl first-changed)
	...
	(let* ((start-dl (car dl))
	  ...
	  (cond ((plusp delta-delta)
		...
		((minusp delta-delta)
		 ...
		 (let ((eff-pos (the fixnum (+ start-pos delta-delta)))
		   ...
		   ;; FIX the move ups of the lines above this one will
		   ;; have queued deletion of the free space left below
		   ;; them, so queue insert of required num lines
	  ...
	  (loop
	    ...
	    (when (or (eq prev last-changed)
		      ;; FIX process all following lines moved by same amount
		      (/= (the fixnum (dis-line-delta car-dl)) curr-delta))
	      (setf prev-delta curr-delta)
	      (return)))))
       ;; FIX already checked new-bit in first case?
       ((logtest flags (logior changed-bit new-bit))
	(if (logtest flags new-bit)
	    (editor-error "ERROR: new in changed/new case"))
	(when (> net-delta 0)
	  ;; "Flush" extra inserts to match position of this dis-line to
	  ;; the position of the associated screen line.
	  (queue (- (dis-line-position (car dl)) cum-inserts)
		 *tty-line-deletions* outs)
	  (queue net-delta *tty-line-deletions* outs)
	  (incf cum-deletes net-delta)
	  (setq net-delta 0))
	(queue (car dl) *tty-line-writes* writes)
	(next-dis-line))
       (t
	...
	(unless (zerop net-delta)
	  ;; FIX add adjustment for the bottom of image (too many/few lines)
...
#|
(defun compute-tty-changes2 (first-changed last-changed modeline-pos)
  (declare (fixnum modeline-pos))
  (trc "c-t-c2 .~%")
  (let* ((dl first-changed)
	 (pos (dis-line-position (car dl)))
	 (flags (dis-line-flags (car dl)))
	 (ins 0) (outs 0) (writes 0) (moves 0)
	 (prev-delta 0) (cum-deletes 0) (net-delta 0) (cum-inserts 0)
	 prev)
    ...
    (loop
      (trc "pos: ~A~%" pos)
      (trc "dl pos: ~A~%" (dis-line-position (car dl)))
      (trc "cum-inserts: ~A~%" cum-inserts)
      (trc "cum-deletes: ~A~%" cum-deletes)
...
;;; terminal doesn't have insert mode or delete mode, we have to blast out
;;; to the end of the dis-line and possibly clear to the end of the screen's
...
;;; These can be found in tty-disp-rt.lisp.
