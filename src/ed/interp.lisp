...
(defun bind-key (name key &optional (kind :global) where)
  "Bind an editor command to some key somewhere.  Name is the string name
   of an editor command, Key is either a key-event or a vector of
   key-events.  Kind is one of :Global, :Mode or :Buffer, and where is the
...
(defun delete-key-binding (key &optional (kind :global) where)
  "Remove an editor key binding somewhere.  Key is either a key-event or a
   vector of key-events.  Kind is one of :Global, :Mode or :Buffer, andl
   where is the mode name or buffer concerned.  Kind defaults to :Global."
