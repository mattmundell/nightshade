...
(defcommand "Exit" (p)
  "Exit the editor, returning to the Lisp top-level read-eval-print loop."
  "Exit the editor, returning to the Lisp top-level read-eval-print loop."
  (declare (ignore p))
  (exit))
...
(defhvar "Next Line Insert"
  "If a true value then \"Next Line\" inserts lines apon reaching the
   buffer end."
  :value t)
...
(defhvar "Scroll Redraw Ratio"
  "This is a ratio of \"inserted\" lines to the size of a window.  When this
   ...
   redraw); however, commands like \"New Line\" and \"Open Line\" will still
...
(defcommand "Extended Command" (p)
  ...
  (multiple-value-bind (name cmd)
		       (prompt-for-keyword (list *command-names*)
					   ...
					   :help "Name of a editor command"
					   :history *extended-command-history*
					   :history-pointer
					   '*extended-command-history-pointer*)
    (let ((function (command-function (getstring name *command-names*)))
	  (bindings (visible-command-bindings cmd)))
      (unwind-protect
	  (funcall function p)
	(if bindings
