...
(defun site-init ()
  ...
  (defhvar "Bell Style"
    "This controls what beeps do in the editor.  Acceptable values are
     :border-flash (which is the default), :feep, :border-flash-and-feep,
     :flash, :flash-and-feep, and NIL (do nothing)."
    :value :border-flash)
  (defhvar "Reverse Video"
    "Paints white on black in window bodies, black on white in modelines.
     Only worked for the X11 configuration of the editor."
    :value nil
    :hooks '(reverse-video-hook-fun))
  (defhvar "Cursor Bitmap File"
    "File to read to setup cursors for editor windows.  The mask is found
     by merging this name with \".mask\"."
    :value "library:ed11.cursor")
  (defhvar "Enter Window Hook"
    "When the mouse enters an editor window, this hook is invoked.  These
     functions take the editor window as an argument."
    :value nil)
  ...
  (defhvar "Default Font"
    "The string name of the font to be used for the editor -- buffer text,
  ...
  (defhvar "Open Paren Highlighting Font"
    "The string name of the font to be used for highlighting open parens.
     The font is loaded when initializing the editor."
...
(defun in-hemlock-standard-input-read (stream &rest ignore)
  ...
  (error "You cannot read off this stream while in the editor -- ~S"
	 stream))
...
(defun schedule-event (time function &optional (repeat t) (absolute nil))
  "Schedule Function to be called at some time.
   ...
   This is a rough mechanism since commands can take an arbitrary amount of
   time to run; the function will be called at the first possible moment
   after the event is due.  Function takes the time that has elapsed since
   the last time it was called (or since it was scheduled for the first
   invocation)."
