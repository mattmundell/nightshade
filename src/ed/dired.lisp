...
(defun symlink-file (source dest)
  ...
  (let* ((ses-name-src (ext:unix-namestring source nil))
	 (ses-name-dest (ext:unix-namestring dest t))
	 (exists-dest-p (unix:unix-file-kind ses-name-dest))
	 ...
	 (wildp-src (wildcardp (file-namestring pname-src)))
	 (wildp-dest (wildcardp (file-namestring pname-dest))))
    (if wildp-dest
	(funcall *error-function*
...
;;;; Delete-File
;;;
(defvar *post-delete-function* nil
  "This function is on the pathname after every delete.")
