...
(defmacro add-hook (place hook-fun &key end)
  ...
  it is interpreted as a Hemlock variable rather than a Lisp variable.  If
...
(defun editor-error (&rest args)
  "This function is called to signal minor errors within Hemlock;
   ...
   progress is always aborted.  This function will only return if it is
   invoked inside an exit hook."  ; FIX will it? (handler returns from block)
