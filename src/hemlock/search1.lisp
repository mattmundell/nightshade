...
(export '(search-pattern search-pattern-p find-pattern replace-pattern
	  new-search-pattern at find-string))
...
;; FIX commented lines from speed testing
(defun search-by-parser (mark parser)
  "Search from Mark for a match according to Parser."
;  (with-mark ((ed::*mark* (copy-mark mark)))
  (with-mark ((ed::*mark* mark))
    (let ((ed::*marks* `((,ed::*mark*)))
...
;	  (node)
      ...
      (loop
;	(ed::message "pre fc")
;	(when (setq node (funcall parser))
;	(when (setq node (buffer-parse-v))
;	(let ((node (buffer-parse-v)))
	(let ((node (funcall parser)))
	  (when node

;	  (ed::message "post fc i")
	    (let ((match-len (count-characters (region pos ed::*mark*))))
	      (move-mark mark pos)
	      (delete-mark pos)
	      (return-from search-by-parser (values node match-len))))
	  )
;	(ed::message "post fc")
	(or (mark-after pos)
	    (progn
	      (delete-mark pos)
	      (return-from search-by-parser)))
	(move-mark ed::*mark* pos)
;	(setq ed::*marks* `((,ed::*mark*)))       ;; FIX already so?
	))))

(defun parser-find-once-forward (pattern line start)
  (let* ((mark (mark line start))
	 (point (copy-mark mark)))
    (multiple-value-bind
	(node len)
	(search-by-parser point (parser-pattern-parser pattern))
      (declare (ignore node))
      (values (mark-line point) (mark-charpos point) len))))

(defun parser-find-once-backward (pattern line start)
  (declare (ignore pattern line start))
  (editor-error "FIX parse backwards"))
...
(define-search-kind :parser (direction pattern old)
  ":parser - Pattern is a list of parser rule requirements, a list
   ...
   do the search."
  (or old (setq old (make-parser-search-pattern)))
  (setf (search-pattern-kind old) :parser
	(search-pattern-direction old) direction
	(search-pattern-pattern old) pattern)
...
;;;
; (defun at (def)
;   "Return a true value if Def matches the text at point.  Def can be a
;    char, string, parser definition, parser rule requirement list or buffer
;    parser function name."
;   (etypecase def
;     (base-char
;      (eq (next-character (current-point)) def))
;     (vector
;      (with-mark ((mark (current-point)))
;        (loop for c across def
; 	 do
; 	 (or (eq (next-character mark) c)
; 	   (return-from at nil))
; 	 (mark-after mark)))
;      t)
;     ((or cons symbol)
